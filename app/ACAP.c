...
static size_t binary_append_callback(char* ptr, size_t size, size_t nmemb, void* userdata) {
    size_t bytes = size * nmemb;
    ACAP_Binary_Buffer* buf = (ACAP_Binary_Buffer*)userdata;
...
unsigned char* ACAP_VAPIX_Get_Binary(const char* endpoint, size_t* out_size) {
    ...
    *out_size = 0;
    ...
    curl_easy_setopt(VAPIX_CURL, CURLOPT_HTTPGET, 1L);
    curl_easy_setopt(VAPIX_CURL, CURLOPT_WRITEFUNCTION, binary_append_callback);
    ...
    if (res != CURLE_OK || http_code >= 300 || !buf.data || buf.size == 0) {
        free(buf.data);
        return NULL;
    }
    *out_size = buf.size;
